...
class Payment(Base):
    ...
    stripe_payment_id = Column(String(255), unique=True, index=True, nullable=False)
    amount = Column(Numeric(10, 2), nullable=False)  # Store as decimal for precision
    currency = Column(String(3), nullable=False, default="usd")  # ISO 4217 currency code
    status = Column(String(50), nullable=False)  # e.g., 'pending', 'succeeded', 'failed'
    payment_method = Column(String(50))  # e.g., 'card', 'bank_transfer'
    created_at = Column(DateTime(timezone=True), server_default=func.now())

    # Relationships
    user = relationship("User", back_populates="payments")
    email_receipts = relationship("EmailReceipt", back_populates="payment")


class EmailReceipt(Base):
    """Email receipt model for tracking payment receipt emails."""
    ...
    # Relationships
    payment = relationship("Payment", back_populates="email_receipts")
